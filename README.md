...
- **YAML 驱动**：每个有效的 `src/content/**/*.yaml` 文件都会成为首页中的一条路线图，并可用目录按主题分类。
- **路线发现**：支持按目录分类、关键词和主题标签组合筛选，并自动统计路线、节点与标签数量。
...
该命令会打开独立编辑器并递归读取 `src/content/**/*.yaml`。保存按钮或 `⌘S`（Windows/Linux 使用 `Ctrl+S`）会直接写回对应分类目录；新建路线默认保存在 `src/content/other`。
...
2. 直接在 `src/content/<category>/*.yaml` 中新增或修改文件，再运行测试和构建检查。

现有分类目录包括：

| 目录 | 内容 |
| --- | --- |
| `ai` | AI 工具、Agent 与应用框架 |
| `backend` | 服务端技术与框架 |
| `career` | 产品、设计与管理岗位 |
| `computer-science` | 计算机科学基础 |
| `data` | 数据分析、数据库与 SQL |
| `devops` | 系统、容器、云原生与 DevSecOps |
| `frontend` | Web 与前端技术 |
| `languages` | 编程语言 |
| `mobile` | 移动端开发 |
| `security` | 安全专题 |
...
首页分类直接由文件所在目录推导，不需要在 YAML 中重复填写分类字段。将文件移动到另一个分类目录后，主页筛选和路线卡片会自动更新。
...
1. Vite 在构建时递归导入 `src/content/**/*.yaml` 的原始文本。
